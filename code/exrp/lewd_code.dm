...
/mob/proc/moan()
	...
	if (gender == FEMALE && prob(25))
		playsound(get_turf(src), "code/exrp/sounds/love/shot[rand(1, 8)].ogg", 90, 1, 0)
	else
		playsound(get_turf(src), "code/exrp/sounds/exrp/interactions/moan_[gender == FEMALE ? "f" : "m"][moan].ogg", 70, 1, 0)
...
/mob/proc/cum(var/mob/living/partner, var/target_orifice)
	...
	if(gender == MALE)
		if (prob(75))
			playsound(loc, "code/exrp/sounds/exrp/interactions/final_m[rand(1, 3)].ogg", 90, 1, 0)
		else
			playsound(loc, "code/exrp/sounds/gachi/penetration_[rand(1, 2)].ogg", 90, 1, 0)
	else if(gender == FEMALE)
		if (prob(75))
			playsound(loc, "code/exrp/sounds/exrp/interactions/final_f[rand(1, 5)].ogg", 70, 1, 0)
		else
...
/mob/living/proc/do_sex(var/mob/living/partner, var/action_to_do) // собак ебать будете в другом билде
	...
	switch(action_to_do)
		if ("do_oral")
			...
			stp = "code/exrp/sounds/exrp/interactions/bj[rand(1, 11)].ogg"
		...
		if ("do_facefuck")
			lust_increase = 10
			c_target = CUM_TARGET_MOUTH
			stp = "code/exrp//sounds/exrp/interactions/oral[rand(1, 2)].ogg"
		...
		if ("do_anal")
			...
			stp = "code/exrp/sounds/exrp/interactions/bang[rand(1, 3)].ogg"
		...
		if ("do_vaginal")
			...
			stp = "code/exrp/sounds/exrp/interactions/champ[rand(1, 2)].ogg"
		...
		if ("do_fingering")
			lust_increase = 10
			c_target = null
			stp = "code/exrp/sounds/exrp/interactions/champ_fingering.ogg"
		...
		if ("do_mountface")
			lust_increase = 1
			c_target = null
			stp = "code/exrp/sounds/exrp/interactions/squelch[rand(1, 3)].ogg"
		...
		if ("do_grindface")
			lust_increase = 1
			c_target = null
			stp = "code/exrp/sounds/exrp/interactions/foot_dry[rand(1, 4)].ogg"
		...
		if ("do_grindmouth")
			lust_increase = 1
			c_target = null
			stp = "code/exrp/sounds/exrp/interactions/foot_wet[rand(1, 3)].ogg"
			if(src.get_item_by_slot(ITEM_SLOT_FEET) != null)
		...
		if ("do_nuts")
			lust_increase = 1
			c_target = CUM_TARGET_MOUTH
			stp = "code/exrp/sounds/exrp/interactions/nuts[rand(1, 4)].ogg"
		...
		if ("do_thighs")
			...
			stp = "code/exrp/sounds/exrp/interactions/[file].ogg"
